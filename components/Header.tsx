...
const Header: React.FC<HeaderProps> = ({ onHelpClick, onSettingsClick, activeView }) => {
    return (
        <header className="bg-slate-950/80 backdrop-blur-2xl sticky top-0 z-40 border-b border-slate-900 shadow-2xl safe-area-top">
            <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12">
                <div className="flex items-center justify-between h-16 md:h-24">
                    <div className="flex items-center gap-3 md:gap-6 group cursor-pointer" onClick={() => window.location.reload()}>
                         <div className="flex items-center justify-center w-10 h-10 md:w-16 md:h-16 text-brand-primary rounded-2xl bg-slate-900 border border-slate-800 shadow-inner transition-all group-hover:border-brand-primary/50 group-hover:shadow-[0_0_30px_rgba(14,165,233,0.15)] relative overflow-hidden">
                        ...
                        <div className="flex flex-col -space-y-1">
                            ...
                            <span className="text-[7px] md:text-[10px] font-bold text-slate-600 uppercase tracking-[0.3em] font-mono truncate">Relational Dynamics Forensic Suite</span>
                        </div>
                    </div>
                    ...
                    <nav className="flex items-center gap-2 md:gap-4">
                        <HeaderButton 
                            ...
                            label="Glossar" 
                        />
                        {/* Fix: removed undefined onSettingsChange and corrected duplicate onClick attribute */}
                        <HeaderButton 
...
const HeaderButton = ({ onClick, active, icon, label }: any) => (
    <button 
        onClick={onClick} 
        className={`flex items-center justify-center w-10 h-10 md:w-auto md:px-6 md:py-3.5 md:gap-3 transition-all rounded-2xl border-2 font-black uppercase tracking-widest text-[10px] ${active ? 'text-brand-primary bg-brand-primary/10 border-brand-primary/30 shadow-[0_0_20px_rgba(14,165,233,0.15)]' : 'text-slate-500 border-slate-900/50 hover:text-brand-primary hover:border-brand-primary/20 hover:bg-slate-900/50'}`}
    >
        {icon}
